...
const Home: NextPage = () => {
  ...
  const queryAllJobs = () => {
    console.log(queryJobs.data?.map((x => ' Job Name: ' + x.id)));
    return (
      <div >
        <div className="gap-2 justify-center flex-col row-span-2">
          {JSON.stringify(queryJobs.data?.filter(x => x.id))}
          
          <button
          onClick={() => {
            deleteJobMutation({id:'clbifdjpq0000wdjwvy3kb01s'});
    ...
    <>
      ...
      <main className="mx-auto my-12 nav-w-3xl  bg-slate-300 p-12"    >
        ...
        <div>
          <button
            onClick={() => {
            //  const result= trpc.jobs.deleteJob();
            }}
            type='button'
            className="flex flex-row items-center gap-2 bg-blue-400 text-sm rounded-md transition p-2 hover:bg-blue-500"
          >
